...
class EmbeddingManager:
    ...
    def __init__(
        ...
        if model is not None:
            ...
            self._embedding_config.model_name = model_name
    ...
    def _load_model(self, device=None):
        ...
        try:
            return SentenceTransformer(self._embedding_config.model_name, device=device)
        except Exception as e:
            raise RuntimeError(
                f"Failed to load embedding model '{self._embedding_config.model_name}'. "
                f"Ensure the model is available. Error: {e}"
            )

    def embed_text(self, text: str) -> np.ndarray:
        """Embed a single text with instruction prefix."""
        prefixed_text = self._embedding_config.instruction_prefix + text
    ...
    def embed_batch(self, texts: List[str], show_progress: bool = True) -> np.ndarray:
        ...
        prefixed_texts = [
            self._embedding_config.instruction_prefix + text for text in texts
        ]
